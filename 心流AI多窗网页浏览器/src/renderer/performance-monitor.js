...
class PerformanceMonitor {
    ...
    collectMetrics() {
        ...
        this.cleanupOldMetrics(); // 清理旧的指标数据
    ...
    cleanupOldMetrics() {
        const now = Date.now();
        const maxAge = 60000; // 保留最近1分钟的数据
        
        // 清理旧的慢操作记录
        this.metrics.ui.slowOperations = this.metrics.ui.slowOperations.filter(op => 
            now - op.timestamp < maxAge
        );
        
        // 限制慢操作数组的最大长度
        if (this.metrics.ui.slowOperations.length > 20) {
            this.metrics.ui.slowOperations = this.metrics.ui.slowOperations.slice(-20);
        }
    }
            ...
            try {
                const longTaskObserver = new PerformanceObserver((list) => {
                    list.getEntries().forEach(entry => {
                        if (entry.duration > 100) { // 提高阈值到100ms，减少误报
    ...
    getOptimizationRecommendations() {
        ...
        // 内存建议
        if (this.metrics.memory.current > 150) { // 提高内存阈值
        ...
        if (errorRate > 0.2) { // 提高错误率阈值
            recommendations.push('网络错误率较高，请检查网络连接');
        }
        
        // UI建议 - 检查最近的慢操作
        const recentSlowOps = this.metrics.ui.slowOperations.filter(op => 
            Date.now() - op.timestamp < 30000 // 只考虑最近30秒的慢操作
        );
        
        if (recentSlowOps.length > 8) { // 提高阈值并只考虑最近的操作
