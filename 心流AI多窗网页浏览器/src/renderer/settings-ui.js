...
class SettingsUI {
    ...
    renderContent() {
        ...
        content.innerHTML = `
            ...
            ${this.currentSection === 'aiServices' ? this.renderSessionTools() : ''}
        `;
        ...
        // 绑定会话管理工具事件
        if (this.currentSection === 'aiServices') {
            this.bindSessionToolsEvents();
        }
    ...
    // 渲染会话管理工具
    renderSessionTools() {
        const services = this.getAllServicesForSessionTools();
        const optionsHtml = services.map(s => `<option value="${s.id}">${s.name} (${s.id})</option>`).join('');
        return `
            <div class="session-tools">
                <div class="session-tools-header">会话管理</div>
                <div class="session-tools-body">
                    <div class="session-tools-row">
                        <label>选择服务</label>
                        <select id="sessionServiceSelect">${optionsHtml}</select>
                    </div>
                    <div class="session-tools-row">
                        <button class="btn btn-danger" id="clearServiceSessionBtn">清除该服务会话数据</button>
                    </div>
                    <p class="session-tools-tip">清除后，该服务的登录信息和本地数据将被移除，需要重新登录。</p>
                </div>
            </div>
        `;
    }
    ...
    // 绑定会话管理工具事件
    bindSessionToolsEvents() {
        const btn = this.modal.querySelector('#clearServiceSessionBtn');
        const sel = this.modal.querySelector('#sessionServiceSelect');
        if (!btn || !sel) return;

        btn.addEventListener('click', async () => {
            const serviceId = sel.value;
            if (!serviceId) return;
            const confirmed = confirm(`确定要清除服务【${serviceId}】的登录会话与本地数据吗？`);
            if (!confirmed) return;
            try {
                const res = await window.electronAPI.clearSessionData(serviceId, {
                    types: ['cookies','localstorage','indexdb','serviceworkers','caches'],
                    includeCache: true
                });
                if (res && res.success) {
                    if (window.notificationSystem) {
                        window.notificationSystem.success(`服务 ${serviceId} 的会话数据已清除`);
                    }
                } else {
                    const msg = res && res.error ? res.error : '未知错误';
                    if (window.notificationSystem) {
                        window.notificationSystem.error(`清除失败：${msg}`);
                    }
                }
            } catch (error) {
                console.error('清除会话数据失败:', error);
                if (window.notificationSystem) {
                    window.notificationSystem.error('清除失败：' + (error.message || String(error)));
                }
            }
        });
    }

    // 获取所有服务（默认+自定义）供会话工具使用
    getAllServicesForSessionTools() {
        const defaults = [
        ...
        let customs = [];
        try {
            const raw = localStorage.getItem('customAIServices');
            if (raw) {
                const parsed = JSON.parse(raw);
                customs = Array.isArray(parsed) ? parsed.map(s => ({ id: s.id, name: s.name || s.id })) : [];
            }
        } catch (e) {
            console.warn('读取自定义服务失败:', e);
        }
        // 去重：以id为准
        ...
        const sections = {
            ...
            aiServices: {
                ...
                fields: [
                    ...
                    {
                        key: 'rememberLoginSessions',
                        label: '记住登录状态',
                        type: 'checkbox',
                        description: '保留各服务的持久化会话（Cookies、本地存储等），避免每次都需要重新登录'
                    },
                    {
                        key: 'defaultService',
                        label: '默认服务',
    ...
    injectStyles() {
        ...
        style.textContent = `
            .settings-modal {
                ...
                right: 0;
                bottom: 0;
                background: linear-gradient(135deg, rgba(227, 242, 253, 0.8) 0%, rgba(187, 222, 251, 0.8) 50%, rgba(144, 202, 249, 0.8) 100%);
                display: none;
                align-items: center;
